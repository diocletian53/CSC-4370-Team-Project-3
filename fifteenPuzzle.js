...
function showTable() {
    ...
    for (var i = 0; i < rows; i++) { //for loop for rows
        outputString += "<tr>"; //table row
        for (var j = 0; j < columns; j++) { //for loop for columns 
            if (arrayForBoardDisplay[i][j] === 0) { // blank tile
                outputString += "<td class=\"blank\"> </td>"; // blank tile class
            } else {
                outputString += "<td class=\"tile\" onclick=\"moveThisTile(" + i + ", " + j + ")\">" + arrayForBoard[i][j] + "</td>";
            }
        }
        outputString += "</tr>"; // closing each row tag
    }
...
function moveThisTile(tableRow, tableColumn) {
    ...
    for (var i = 0; i < directions.length; i++) {
        if (checkIfMoveable(tableRow, tableColumn, directions[i])) { //checks if the tile is moveable
            incrementMoves();
